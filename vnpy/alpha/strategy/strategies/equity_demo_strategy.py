...
class EquityDemoStrategy(AlphaStrategy):
    ...
    def on_bars(self, bars: dict[str, BarData]) -> None:
        ...
        if not cash:
            print("Cash is NaN, setting to 0")
            cash = 0
        # Buy rebalancing
        if buy_symbols:
            buy_value: float = (
            ...
            for vt_symbol in buy_symbols:
                ...
                # Calculate volume to buy with NaN check
                calculated_volume = buy_value / buy_price
                if calculated_volume != calculated_volume:  # Check for NaN
                    print(
                        f"Calculated volume is NaN for {vt_symbol}, skipping",
                        buy_value,
                        buy_price,
                        self.cash_ratio,
                        cash,
                ...
                buy_volume: float = round_to(
                    calculated_volume, self.min_volume
