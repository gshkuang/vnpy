...
class Alpha158(AlphaDataset):
    ...
    def __init__(
        ...
        for w in windows:
            self.add_feature(
                f"rsv_{w}", (c - ts_min(l, w)) / (ts_max(h, w) - ts_min(l, w) + 1e-12)
            )
        ...
        for w in windows:
            self.add_feature(
                f"imxd_{w}", (ts_argmax(h, w, period) - ts_argmin(l, w, period)) / w
            )
        ...
        for w in windows:
            self.add_feature(
                f"cntd_{w}",
                ts_mean(c > ts_delay(c, 1), w) - ts_mean(c < ts_delay(c, 1), w),
            )

        for w in windows:
            self.add_feature(
                f"sump_{w}",
                ts_sum(ts_greater(c - ts_delay(c, 1), 0), w)
                / (ts_sum(ts_abs(c - ts_delay(c, 1)), w) + 1e-12),
            )

        for w in windows:
            self.add_feature(
                f"sumn_{w}",
                ts_sum(ts_greater(ts_delay(c, 1) - c, 0), w)
                / (ts_sum(ts_abs(c - ts_delay(c, 1)), w) + 1e-12),
            )
        ...
        for w in windows:
            self.add_feature(
                f"wvma_{w}",
                ts_std(ts_abs(c / ts_delay(c, 1) - 1) * v, w)
                / (ts_mean(ts_abs(c / ts_delay(c, 1) - 1) * v, w) + 1e-12),
            )

        for w in windows:
            self.add_feature(
                f"vsump_{w}",
                ts_sum(ts_greater(v - ts_delay(v, 1), 0), w)
                / (ts_sum(ts_abs(v - ts_delay(v, 1)), w) + 1e-12),
            )

        for w in windows:
            self.add_feature(
                f"vsumn_{w}",
                ts_sum(ts_greater(ts_delay(v, 1) - v, 0), w)
                / (ts_sum(ts_abs(v - ts_delay(v, 1)), w) + 1e-12),
            )
        ...
        base_proxy = {"open": o, "high": h, "low": l, "close": c, "vwap": vwap}.get(
            base_field, c
        )
        label = (
            ts_delay(base_proxy, forward_shift) / ts_delay(base_proxy, base_shift) - 1
        )
