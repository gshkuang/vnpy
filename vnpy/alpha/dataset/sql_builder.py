...
class DuckDBSQLBuilder:
    ...
    def build_global_robust_stats(self, where_clause: str) -> tuple[str, str]:
        ...
        # MAD SQL
        mad_exprs = [f"median(abs({c} - med.{c}_median)) AS {c}_mad" for c in self.features]
        mad_sql = f"""
    ...
    def build_cross_sectional_robust_stats(self, where_clause: str) -> tuple[str, str]:
        ...
        # 按日期分组的中位数SQL
        median_exprs = ["datetime"] + [f"median({c}) AS {c}_median" for c in self.features]
        median_sql = f"""
        SELECT {', '.join(median_exprs)}
        FROM read_parquet('{self.glob_path}')
        {where_clause}
        ...
        # 按日期分组的MAD SQL
        mad_exprs = ["m.datetime"] + [f"median(abs(t.{c} - m.{c}_median)) AS {c}_mad" for c in self.features]
        mad_sql = f"""
    ...
    # ==== 新增：数据切分查询 ====
    def build_period_select(self, selected_cols: list[str], where_clause: str, shuffle: bool = True) -> str:
        """
        ...
        order_clause = "ORDER BY random()" if shuffle else "ORDER BY datetime, vt_symbol"
        return f"""
        SELECT {cols_csv}
        FROM read_parquet('{self.glob_path}')
        {where_clause}
...
def build_stats_sql(
    ...
    if stats_type == "global":
        ...
        if method == "zscore":
            ...
            median_sql, mad_sql = builder.build_cross_sectional_robust_stats(where_clause)
