...
from vnpy.alpha.dataset.ts_function import (
    ts_slope as ts_slope_old,
    ts_rsquare as ts_rsquare_old,
    ts_resi as ts_resi_old,
)


def make_synthetic_df(num_symbols: int = 5, rows_per_symbol: int = 10000) -> pl.DataFrame:
    ...
    for i in range(num_symbols):
        ...
        noise = pl.Series("noise", pl.arange(0, rows_per_symbol, eager=True)).cast(pl.Float64)
...
def ts_slope_vec(feature: FeatProxy, window: int) -> FeatProxy:
    ...
    lf = feature.df.with_columns(
        pl.col("datetime").cast(pl.Float64).alias("x")
    ).with_columns(
        (pl.col("x")).rolling_sum(window, min_samples=1).over("vt_symbol").alias("Sx"),
        (pl.col("data")).rolling_sum(window, min_samples=1).over("vt_symbol").alias("Sy"),
        (pl.col("x") * pl.col("x")).rolling_sum(window, min_samples=1).over("vt_symbol").alias("Sxx"),
        (pl.col("x") * pl.col("data")).rolling_sum(window, min_samples=1).over("vt_symbol").alias("Sxy"),
        # n = min(window, index within group + 1). Compute index by datetime offset from group min
        (pl.min_horizontal(
            pl.lit(window),
            (pl.col("datetime") - pl.col("datetime").min().over("vt_symbol")).cast(pl.Float64) + 1
        )).alias("n"),
    ).with_columns(
        (
...
def ts_rsquare_vec(feature: FeatProxy, window: int) -> FeatProxy:
    ...
    lf = feature.df.with_columns(
        ...
        pl.when((pl.col("var_x") <= 0) | (pl.col("var_y") <= 0))
          .then(None)
          .otherwise(((pl.col("cov") * pl.col("cov")) / (pl.col("var_x") * pl.col("var_y"))))
          .alias("data")
    )
    return FeatProxy(lf)
...
def bench_one(name: str, old_fn, new_fn, feature: FeatProxy, window: int) -> Tuple[float, float, pl.DataFrame]:
    ...
    joined = old_df.rename({"data": f"{name}_old"}).join(
        new_df.rename({"data": f"{name}_new"}), on=["datetime", "vt_symbol"], how="inner"
    )
...
def accuracy_stats(df: pl.DataFrame, name: str) -> None:
    ...
    mism = df2.filter(pl.col(diff_col) > 1e-6).select(["datetime", "vt_symbol", old_col, new_col, diff_col])
    if mism.height > 0:
...
def main() -> None:
    ...
    # slope
    s_old, s_new, s_join = bench_one("slope", ts_slope_old, ts_slope_vec, feature, window)
    print(f"[TIME] slope old={s_old:.4f}s new={s_new:.4f}s speedup={s_old/s_new if s_new>0 else float('inf'):.2f}x")
    accuracy_stats(s_join, "slope")

    # rsquare
    r_old, r_new, r_join = bench_one("rsquare", ts_rsquare_old, ts_rsquare_vec, feature, window)
    print(f"[TIME] rsquare old={r_old:.4f}s new={r_new:.4f}s speedup={r_old/r_new if r_new>0 else float('inf'):.2f}x")
    accuracy_stats(r_join, "rsquare")

    # residual
    e_old, e_new, e_join = bench_one("resi", ts_resi_old, ts_resi_vec, feature, window)
    print(f"[TIME] resi old={e_old:.4f}s new={e_new:.4f}s speedup={e_old/e_new if e_new>0 else float('inf'):.2f}x")
